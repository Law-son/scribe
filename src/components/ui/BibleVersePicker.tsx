...
export function BibleVersePicker({ value, onChange, label = "Scripture Reference" }: Props) {
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      >
        {value ? (
          <span className="text-navy font-medium">{value.reference} <span className="font-normal text-navy/50">({value.translation})</span></span>
        ) : (
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="rounded-xl bg-cream border border-cream-dark px-4 py-3 min-h-[80px] flex flex-col justify-between">
              {loading ? (
                ...
                <>
                  <p className="text-sm font-body text-navy-dark leading-relaxed italic">&ldquo;{currentVerse.text}&rdquo;</p>
                  <p className="text-[11px] text-gold-dark font-body font-medium mt-2">— {book.name} {chapter}:{verse} ({translation})</p>
                </>
              ) : (
