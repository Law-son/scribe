...
export default async function AdminSermonsPage() {
  await connectDB();
  const sermons = await Sermon.find().sort({ createdAt: -1 }).limit(50).lean();
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-navy/60 font-body mt-1">{sermons.length} total</p>
        </div>
        <Link
              ...
              {sermons.map((s) => (
                <tr key={s._id.toString()} className="hover:bg-cream-light/50 transition-colors">
                  ...
                  <td className="px-5 py-4">
                    <Badge variant={s.status === "published" ? "green" : "gray"}>
                      {s.status}
                    </Badge>
                  </td>
